...
const BottomNav: React.FC<Props> = ({ activeTab, onTabChange }) => {
  return (
    <nav
      className="shrink-0 flex bg-secondary/90 backdrop-blur-md border-t border-border/50 z-40 px-2 pb-safe"
      role="navigation"
    ...
    >
      {TABS.map(tab => {
        ...
        return (
          <button
            key={tab.id}
            className={`flex-1 flex flex-col items-center justify-center py-2.5 min-h-[56px] relative cursor-pointer outline-none transition-colors ${isActive ? 'text-accent' : 'text-muted-foreground hover:text-foreground'}`}
