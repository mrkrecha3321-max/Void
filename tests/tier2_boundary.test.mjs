...
describe('Tier 2: Boundary & Corner Cases Verification', () => {
  it('Test 2.1: Invalid/extreme RSSI values (0 dBm, positive RSSI, NaN, null, -120 dBm) return safe fallback distances without crashing', () => {
    ...
    // Positive RSSI (10 dBm) -> safe fallback (5.0m)
    ...
    // null / undefined -> safe fallback (5.0m)
    ...
    // -120 dBm (extreme low signal) -> clamped max distance (100.0m) without crashing
  ...
  it('Test 2.2: Malformed Peer ID inputs (empty string, null, invalid characters) return validation error', () => {
    ...
    // Invalid special characters (e.g. script injection or wildcards)
  ...
  it('Test 2.3: Malformed presence packet JSON (missing msgType, invalid Base64 senderPubkey, syntax error) is safely rejected', () => {
    ...
    // Invalid Base64 senderPubkey (too short / corrupt string)
    const badKeyObj = {
      msgType: 'presence',
      senderId: 'VX-TEST1',
  ...
  it('Test 2.9: LRU deduplication cache drops duplicate message IDs and caps history length', () => {
    ...
    // 1. Process first unique message
    ...
    // 3. Add more unique messages exceeding capacity (max 3)
    ...
    // Add 4th unique message -> msg-100 (oldest) is evicted
    ...
    // msg-100 should now be treated as new if received again (since evicted)
